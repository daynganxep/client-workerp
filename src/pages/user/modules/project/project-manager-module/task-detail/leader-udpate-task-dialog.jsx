...
function LeaderUpdateTaskDialog({ task, refetch }) {
    ...
    const form = useForm({
        ...
        defaultValues: _.pick(task, ["title", "description", "projectId", "assignees", "priority", "status", "dueDate",])
    });
    ...
    const mutation = useMutation({
        mutationFn: async (data) => {
            const [res, err] = await TaskService.updateTask(taskId, data);
            console.log({ res })
            if (err) throw new Error(err.code);
            return res;
        },
...
export default LeaderUpdateTaskDialog;